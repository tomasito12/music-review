...
from music_review.dashboard.playlist_export import format_tune_my_music_txt
...
def test_display_rows_align_with_tune_my_music_txt_export() -> None:
    """Table rows and TXT export share the same artist and title fields."""
    suggestions = [
        PlaylistSuggestion(
            review_id=1,
            artist="Artist A",
    ...
    assert format_tune_my_music_txt(suggestions) == (
        f"{rows[0]['Künstler']} - {rows[0]['Titel']}"
    )
